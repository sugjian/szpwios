...
(function($) {
	...
	var preventDefaultException = /^(INPUT|TEXTAREA|BUTTON|SELECT|A)$/;
	/**
	 * mui delegate events
	 * @param {type} event
	 * @param {type} selector
		...
		init: function(element, options) {
			...
			this.options = $.extend(true, {
				...
				preventDefaultException: {
					tagName: /^(INPUT|TEXTAREA|BUTTON|A|SELECT|VIDEO)$/
				},
				momentum: true,
				...
				case 'drag':
					...
					if (!this.isDragging && Math.abs(this.lastX - this.startX) > this.options.dragThresholdX && (detail.direction === 'left' || (detail.direction === 'right'))) {
						...
						if (this.offCanvas && this.scroller) {
							...
							this.isDragging = this.isShown();	//zhaold true->false; 屏蔽拖动
