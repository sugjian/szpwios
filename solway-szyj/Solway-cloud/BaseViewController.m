...
/*----------Test-----------*/
    ...
    //NSString *url = [[NSString alloc] initWithFormat:@"http://itunes.apple.com/lookup?id=%@",@"1179719096"];
    
    //中国区上架获取连接
    NSString *url = [[NSString alloc] initWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@",@"1179719096"];
    [self Postpath:url];
...
-(void)receiveData:(id)sender
{
    ...
    [appDelegate.bridge callHandler:@"setNewVersion" data:[sender objectForKey:@"version"] responseCallback:^(id response) {
        NSLog(@"\n---JS[setNewVersion] responded: %@\n\n", response);
    }];
    
    [appDelegate.bridge callHandler:@"setOldVersion" data:_oldVersion responseCallback:^(id response) {
        NSLog(@"\n---JS[setOldVersion] responded: %@\n\n", response);
    }];
